...
class GreatWall_Pro_Timeline extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'greawall_pro_timeline_widget', 'description' => __( 'Display your timeline.', 'greatwall-pro') );
        parent::__construct(false, $name = __('Home - Timeline', 'greatwall-pro'), $widget_ops);
		$this->alt_option_name = 'greawall_pro_timeline_widget';
	...
	<p><?php _e('This widget requires you to re-import the settings file and create some timeline events.', 'greatwall-pro'); ?></p>
	<p>
	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'greatwall-pro'); ?></label>
	<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
	</p>
	<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of timeline events to show (-1 shows all of them):', 'greatwall-pro' ); ?></label>
	<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
    <p><label for="<?php echo $this->get_field_id('see_all'); ?>"><?php _e('The URL for your button [In case you want a button below your timeline block]', 'greatwall-pro'); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all_text'); ?>"><?php _e('The text for the button [Defaults to <em>See all our events</em> if left empty]', 'greatwall-pro'); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php _e( 'Enter the slug for your category or leave empty to show all events.', 'greatwall-pro' ); ?></label>
			...
			<?php if ( $title ) echo $before_title . $title . $after_title; ?>
				...
				<?php if ($see_all != '') : ?>
					<a href="<?php echo esc_url($see_all); ?>" class="roll-button more-button">
						...
						<?php else : ?>
							<?php echo __('See all our timeline', 'greatwall-pro'); ?>
						<?php endif; ?>
					</a>
				<?php endif; ?>				
	<?php
