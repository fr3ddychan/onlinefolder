 ...
 * @since      1.0.0
 */
class greatwall_pro_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-greatwall_pro-popular widget_posts_thumbnail',
			'description' => __( 'Display popular posts by comments with thumbnails.', 'greatwall-pro' )
		);

		// Create the widget.
		parent::__construct(
			'greatwall_pro-popular',                                   // $this->id_base
			__( '&raquo; Popular Posts', 'greatwall-pro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'greatwall_pro_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'greatwall_pro_popular_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'greatwall_pro_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Popular Posts', 'greatwall-pro' ),
			'limit'     => 5,
			'show_views' => true
		);
