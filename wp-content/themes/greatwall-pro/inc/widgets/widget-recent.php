 ...
 * @since      1.0.0
 */
class greatwall_pro_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-greatwall_pro-recent widget_posts_thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'greatwall-pro' )
		);

		// Create the widget.
		parent::__construct(
			'greatwall_pro-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'greatwall-pro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'greatwall_pro_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'greatwall_pro_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'greatwall_pro_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'greatwall-pro' ),
			'limit'     => 5,
			'show_date' => true
		);
